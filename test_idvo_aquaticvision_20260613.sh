...
# EuRoC-like cache, runs selected UW-IDVO modes, and saves evo evaluation plots.
...
LOG_FILE="$RESULT_ROOT/uwidvo_aquaticvision.log"
...
normalize_mode() {
  ...
  case "$upper" in
    ...
    OFF|FIXED|RULE|RULEBASED|LEARNED) printf '%s\n' IDVO ;;
    *) return 1 ;;
  esac
}
...
apply_mode_to_config() {
  ...
  awk '
    !/^[[:space:]]*(UWIDVO\.Mode|InfoSelector\.Enable|InfoKF\.Use|EnableAdaptiveIDVO|AdaptivePolicyType)[[:space:]]*:/
  ' "$src" > "$dst"
  cat >> "$dst" <<EOF

# Generated by test_idvo_aquaticvision_20260613.sh
UWIDVO.Mode: "${mode}"
...
run_one() {
  ...
  if ! exe="$(find_exe "$exe_name")"; then
    ...
    write_row "aquaticvision" "$seq" "$sensor" "$mode" "$run_id" "SKIP" "0" "0" "0" "0" "0.000" "$result_dir" "$cfg" "" "$cache/gt.tum" "" "" "" "missing_executable_$exe_name"
    return 0
  fi

  tag="uwidvo_aquaticvision_${seq_tag}_${sensor}_${mode}_r${run_id}"
  traj="$result_dir/f_${tag}.txt"
  gt="$cache/gt.tum"

  log "[RUN] sequence=$seq sensor=$sensor mode=$mode run=$run_id"
  start_time="$(date +%s)"
  set +e
  (
    cd "$result_dir"
...
PY
  ...
  write_row "aquaticvision" "$seq" "$sensor" "$mode" "$run_id" "$status" "$exit_code" "$elapsed" "$input_frames" "$traj_poses" "$completeness" "$result_dir" "$cfg" "$traj" "$gt" "$result_dir/evo" "$ate" "$rpe" "$notes"
}
  ...
  for seq in $SEQUENCES; do
    for sensor in $SENSORS; do
      for raw_mode in $MODES; do
        mode="$(normalize_mode "$raw_mode")" || { log "[WARN] invalid mode skipped: $raw_mode"; continue; }
        for run_id in $(seq 1 "$RUNS_PER_CASE"); do
          run_one "$seq" "$sensor" "$mode" "$run_id"
        done
      done
    done
  done
