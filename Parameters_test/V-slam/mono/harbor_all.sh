...
# 8 套 YAML 所在目录
CONFIG_DIR="${CONFIG_DIR:-$WS/src/orb_slam3_ros/config/Monocular/Aquacular_harbor/all}"

# 8 套配置名字（必须与 .yaml 文件名对应）
CONFIG_NAMES=(
  #"Aqualoc_harbor_off_00"
  #"Aqualoc_harbor_off_01"
  #"Aqualoc_harbor_off_10"
  "Aqualoc_harbor_off_11"
  # "Aqualoc_harbor_on_00"
  # "Aqualoc_harbor_on_01"
  # "Aqualoc_harbor_on_10"
  # "Aqualoc_harbor_on_11"
)

# 要跑的序列索引（1..7）
#SEQ_INDICES=(1 2 3 4 5 6 7)
SEQ_INDICES=( 1 2 3 5 6 )
...
# 修复点：直接从 zip 内 stats.json 读取，避免解析 evo_res 输出表头导致 rmse=mean
get_metric_from_zip() {
  local zipfile="$1"
  local metric="$2"
...
try:
    with zipfile.ZipFile(zip_path, "r") as z:
        ...
        if not names:
            sys.exit(0)
        names = sorted(names, key=len)  # 选最短路径，通常就是根目录 stats.json
...
run_one_sequence() {
  ...
  # ★ 修复点：这里必须是 8 列，且 printf 格式串不能有任何杂字符
  awk -v s="${SCALE}" '
    $1 ~ /^#/ {next}
    NF>=8 {
      printf("%.9f %s %s %s %s %s %s %s\n", $1/s, $2,$3,$4,$5,$6,$7,$8)
