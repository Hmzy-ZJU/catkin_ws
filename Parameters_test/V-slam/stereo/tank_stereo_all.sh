...
RESULTS_ROOT="${RESULTS_ROOT:-$TANK_ROOT/results_stereo}"
...
LAUNCH_NAME="${LAUNCH_NAME:-tank_stereo.launch}"

# Tank Stereo YAML 所在目录
CONFIG_DIR="${CONFIG_DIR:-$WS/src/orb_slam3_ros/config/Stereo/Tank}"

# 8 种配置名称（与 YAML 文件名对应，不含 .yaml 后缀）
CONFIG_NAMES=(
  "Tank_stereo_off_00"
  "Tank_stereo_off_01"
  "Tank_stereo_off_10"
...
RAW_CSV="${RESULTS_ROOT}/tank_stereo_eval_runs.csv"
...
SUMMARY_CSV="${RESULTS_ROOT}/tank_stereo_eval_summary.csv"
...
run_one_sequence() {
  ...
  echo "============================================================"
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 开始处理序列"
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] YAML       : ${cfg_yaml}"
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] rosbag     : ${bag_file}"
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] GT         : ${gt_file}"
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] UW_TAG     : ${UW_TAG}"
  echo "============================================================"
  ...
  mkdir -p "${work_dir}"
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 评估输出目录: ${work_dir}"

  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 启动 roslaunch..."
  ...
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 等待 ROS master..."
  until rosparam list >/dev/null 2>&1; do
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}]  ROS master 未就绪，等待中..."
    sleep 1
  done

  rosparam set use_sim_time true || true

  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 等待 /orb_slam3/save_traj 服务..."
  timeout 60s bash -c 'until rosservice info /orb_slam3/save_traj >/dev/null 2>&1; do sleep 1; done' || true

  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 开始播放 rosbag: ${bag_file} (rate=${BAG_RATE})"
  rosbag play "${bag_file}" --clock -r "${BAG_RATE}" >/dev/null 2>&1 || true
  sleep 5

  local traj_tag="${cfg_name}_${seq_name}_r${run_id}_${UW_TAG}"

  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 调用 save_traj 服务..."
  rosservice call /orb_slam3/save_traj "{traj_tag: '${traj_tag}'}" || true
  sleep 2
  ...
  if [[ -f "${est_cam_src}" ]]; then
    ...
    cp "${est_cam_src}" "${work_dir}/${traj_tag}_cam_traj.txt"
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 相机轨迹: ${est_cam_dst}"
  else
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] [WARN] 未找到相机轨迹: ${est_cam_src}"
  fi

  if [[ -f "${est_kf_src}" ]]; then
    cp "${est_kf_src}" "${est_kf_dst}"
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 关键帧轨迹: ${est_kf_dst}"
  else
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] [WARN] 未找到关键帧轨迹: ${est_kf_src}"
  fi
  ...
  if [[ -f "${est_map_src}" ]]; then
    ...
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 地图点云: ${est_map_dst}"
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 点云已同步到评估目录: ${est_map_work}"
  else
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] [WARN] 未找到点云: ${est_map_src}"
  fi

  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 关闭 /orb_slam3..."
  rosnode kill /orb_slam3 >/dev/null 2>&1 || true

  if ps -p "${slam_pid}" >/dev/null 2>&1; then
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 杀掉 roslaunch (PID=${slam_pid})..."
    kill "${slam_pid}" >/dev/null 2>&1 || true
    wait "${slam_pid}" 2>/dev/null || true
  fi

  if [[ ! -f "${est_cam_dst}" ]]; then
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] [ERROR] 估计轨迹不存在，跳过评估: ${est_cam_dst}"
    t_end=$(date +%s)
  ...
  if [[ -f "${perf_csv}" ]]; then
    ...
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 性能统计: ${perf_csv}"
  else
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] [WARN] 未找到性能统计: ${perf_csv}"
  fi

  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 开始评估 ATE/RPE..."
  cd "${work_dir}"
  ...
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] Detected est timestamp scale: divide by ${SCALE}"
  ...
  if [[ "${RPE_ENABLED}" -eq 1 ]]; then
    echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] RPE enabled: unit=${RPE_UNIT}, delta=${RPE_DELTA}"
  ...
  echo "[STEREO] [CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] 完成，结果目录: ${work_dir}"
}
...
echo "[INFO] 配置数量: ${#CONFIG_NAMES[@]}"
echo "[INFO] 序列数量: ${#SEQ_NAMES[@]}"
...
echo "[INFO] ====== Tank Stereo 全部实验结束 ======"
echo "[INFO] 每次运行的原始记录在: ${RAW_CSV}"
echo "[INFO] 按中位数汇总的结果在: ${SUMMARY_CSV}"
