...
LAUNCH_NAME="${LAUNCH_NAME:-tank_mono_inertial.launch}"

# Tank YAML 所在目录
CONFIG_DIR="${CONFIG_DIR:-$WS/src/orb_slam3_ros/config/Monocular-Inertial/Tank}"
...
FPS="${FPS:-20}"                  # 这里 Tank 用时间戳，不再依赖 FPS
...
run_one_sequence() {
  ...
  local seq_name="$3"   # 序列名，如 HalfTank_Easy
  ...
  echo "[CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] ROS master 已就绪。"
  ...
  echo "[CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] rosbag 播放结束，准备保存轨迹…"
  sleep 5
  ...
  if ! rosservice info /orb_slam3/save_traj >/dev/null 2>&1; then
    echo "[CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] [WARN] /orb_slam3/save_traj 暂不可用，继续等待 20s"
  ...
  # 4.7.1 GT: CSV -> TUM  (Tank: timestamp[ns], pos_x[m], pos_y[m], pos_z[m], orient_qx, orient_qy, orient_qz, orient_qw)
  # 将纳秒时间戳转换为秒
  awk -F',' '
    NR>1 && NF>=8 {
        ts = $1 / 1e9;
  ...
  evo_traj tum groundtruth.tum --save_plot gt_traj_xz.pdf --plot_mode xz --no_warnings 
  ...
  evo_traj tum est.tum --save_plot est_traj_xz.pdf --plot_mode xz --no_warnings 
  # 4.7.3 时间归一化
  awk 'BEGIN{OFS=" "}
    $1!~/^#/ && NF>=8 {
      if(!seen++){t0=$1}
  ...
  evo_traj tum est_norm.tum --save_plot est_norm_xz.pdf --plot_mode xz --no_warnings 
  evo_traj tum groundtruth_norm.tum --save_plot gt_norm_xz.pdf --plot_mode xz --no_warnings 
  ...
  if [[ "${RPE_ENABLED}" -eq 1 ]]; then
    ...
    echo "[CFG ${cfg_name}] [SEQ ${seq_name}] [RUN ${run_id}] RPE disabled"
  fi
...
########################################
# 5. 主循环：8 个配置 × 8 个序列 × RUNS_PER_SEQ 次
########################################
