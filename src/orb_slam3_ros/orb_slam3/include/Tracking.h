...
public:
    ...
    //分别从 OpenCV 的 FileStorage 读取相机内参/畸变、ORB 提取参数、IMU 器件噪声/对齐等。
    bool ParseCamParamFile(cv::FileStorage &fSettings);
    bool ParseORBParamFile(cv::FileStorage &fSettings);
    bool ParseIMUParamFile(cv::FileStorage &fSettings);

    // Preprocess the input and call Track(). Extract features and performs stereo matching.
    //三个入口函数：接收图像帧（整形/灰度/已矫正等），内部做预处理、特征提取、匹配/深度融合、然后调用 Track()。
    //返回 Sophus::SE3f：当前相机位姿（世界到相机或相机到世界，具体在实现里定义，这里常为 Tcw 或其逆）。
    ...
    //将时间戳有序的 IMU 点（加计/陀螺）入队列，供预积分使用。
    ...
    float GetImageScale();
//当系统启用了回环检测/闭环功能时，才需要这些接口来控制相关线程。
#ifdef REGISTER_LOOP
    void RequestStop();
    bool isStopped();
    void Release();
...
public:
    ...
    int mTrackedFr;
    bool mbStep;
    // === Performance dump flag ===
    bool mbPerfDumped = false;  // 防止重复导出 perf.csv
...
struct RuntimeStats {
    // per-run 统计
    std::vector<double> track_ms;      // 每帧总 tracking 时间
    std::vector<double> ba_ms;         // 每次局部 BA 时间（由 LocalMapping 回传，见步骤 3）
    std::vector<int>    selected_pts;  // 每关键帧/每帧的被选点数（见步骤 2）
    std::vector<double> enh_ms;        // 每帧图像增强耗时 (ms)
    size_t num_kfs = 0;                // 关键帧数量（run 结束时填）
    size_t num_mps = 0;                // 地图点数量（可选）
...
inline void RecordTrackTime(const std::chrono::steady_clock::time_point& t_start)
{
    ...
    // 毫秒计时
    double track_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t_end - t_start).count();
...
void SaveRuntimeStatsCSV(const std::string& filepath);
void ResetRuntimeStats();
// 读取当前进程常驻内存（MB）
static double GetProcessRSS_MB();
