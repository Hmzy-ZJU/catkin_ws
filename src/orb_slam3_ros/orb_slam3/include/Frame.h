...
public:
    ...
    // InfoGain feature-selection flag for each keypoint (size = N)
    // true  : 该特征被信息选点模块选中
    // false : 未被选中
    std::vector<bool> mvInfoSelected;
    // InfoGain candidate flag for each keypoint (size = N)
    std::vector<bool> mvInfoCandidate;
    // Optional: candidate type (size = N)
    // 0 = non-candidate
    // 1 = candidate matched to Map point (Observations() > 0)
    // 2 = candidate matched to VO point   (Observations() == 0)
    std::vector<unsigned char> mvInfoCandType;
    int mnCloseMPs;
