            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        // ===== [计时-开始] 本次 Local BA（惯性）=====
                        auto tLBA0 = std::chrono::steady_clock::now();
                        ...
                        auto tLBA1 = std::chrono::steady_clock::now();
                        double ba_ms = std::chrono::duration_cast<std::chrono::milliseconds>(tLBA1 - tLBA0).count();
                        if (mpTracker) mpTracker->mRtStats.ba_ms.push_back(ba_ms);
                        // std::cout << "[LBA] inertial took " << ba_ms << " ms\n"; // 可选调试
                    ...
                    else
                    {
                            ...
                            double ba_ms = std::chrono::duration_cast<std::chrono::milliseconds>(tLBA1 - tLBA0).count();
                            if (mpTracker) mpTracker->mRtStats.ba_ms.push_back(ba_ms);
                            // std::cout << "[LBA] visual took " << ba_ms << " ms\n"; // 可选调试
