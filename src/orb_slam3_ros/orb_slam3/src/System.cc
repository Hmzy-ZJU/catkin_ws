    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    // 1.检査 SLAM 系统传感器初始化类型，需要用到 sensor;
    cout << endl <<
    ...
    //Check settings file  
    // 2.检査传感器配置文件，也就是相机标定参数等，需要用到 strSettingsFile
    ...
    if(!node.empty() && node.isString() && node.string() == "1.0"){
        ...
        node = fsSettings["System.SavePointCloudToFile"];
        if(!node.empty() && node.isString())
        {
            mStrSavePointCloudToFile = (string)node;
        }
    ...
    //4.根据 ORB 词典实例对象,创建关键帧 DataBase
    ...
    //7.利用 Atlas 创建 Drawer,包括 FrameDrawer和 MapDrawer;
...
void System::Shutdown()
{
    ...
    // ====== new: save point cloud (PLY) ======
    if(!mStrSavePointCloudToFile.empty())
    {
        // 建议：尽量在 LocalMapping/LoopClosing 停下来后保存
        // 你这里原本有等待线程结束的 while 循环，但被注释了。
        // 如果你担心卡住，可以用“有限等待”：
        for(int k=0; k<400; ++k) // 400*5ms=2s
        {
        ...
        Verbose::PrintMess("PointCloud saving to file " + mStrSavePointCloudToFile, Verbose::VERBOSITY_NORMAL);
        SaveMapPointsPLY(mStrSavePointCloudToFile, nullptr, true);
    }
    // ====== end ======
...
void System::RegisterEnhanceTime(double ms)
{
    if(mpTracker)
        mpTracker->RegisterEnhanceTime(ms);
}
...
void System::SaveMapPointsPLY(const std::string &filename, Map* pMap, bool bOnlyGood)
{
    // Choose map
    Map* pUseMap = pMap;
    if(!pUseMap)
    {
        if(!mpAtlas) return;
        pUseMap = mpAtlas->GetCurrentMap();
        if(!pUseMap) return;
    }

    std::vector<MapPoint*> vMPs = pUseMap->GetAllMapPoints();

    // Count valid points first (to write correct PLY header)
    size_t nValid = 0;
    for(auto* pMP : vMPs)
    {
        if(!pMP) continue;
        if(bOnlyGood && pMP->isBad()) continue;
        nValid++;
    }
    ...
    if(!f.is_open())
    {
        std::cerr << "[System] Cannot open file to save PLY: " << filename << std::endl;
        return;
    }
    ...
    f.close();
    std::cout << "[System] Saved MapPoints PLY: " << filename
              << " (valid=" << nValid << ", total=" << vMPs.size() << ")\n";
}
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
    ...
    cout << endl << "End of saving trajectory to " << filename << " ..." << endl;
        // ===== [Perf Export] Save performance CSV =====
    const char* home = std::getenv("HOME");
    std::string ros_home = home ? (std::string(home) + "/.ros/") : std::string("/tmp/");

    // 从 filename 提取前缀（去掉目录与 _cam_traj.txt）
    ...
    std::string stats_csv = ros_home + base + "_perf.csv";

    if (mpTracker)
    {
        mpTracker->SaveRuntimeStatsCSV(stats_csv);
        std::cout << "[Perf] Stats saved to " << stats_csv << std::endl;
    }
    // ===== [Map Export] Save MapPoints PLY =====
    if(pBiggerMap)
    {
        std::string map_ply = ros_home + base + "_map.ply";
        std::cout << "[SavePCD] SaveMapPointsPLY -> " << map_ply << std::endl;
        this->SaveMapPointsPLY(map_ply, pBiggerMap, true);
        std::cout << "[SavePCD] done" << std::endl;
    }
    else
    {
        std::cerr << "[SavePCD] No valid map found, skip point cloud save." << std::endl;
    }
...
void System::SaveTrajectoryEuRoC(const string &filename, Map* pMap)
{
    ...
    if(pMap)
    {
        std::string map_ply = ros_home + base + "_map.ply";
        std::cout << "[SavePCD] SaveMapPointsPLY -> " << map_ply << std::endl;
        this->SaveMapPointsPLY(map_ply, pMap, true);
        std::cout << "[SavePCD] done" << std::endl;
    }
    else
    {
        std::cerr << "[SavePCD] pMap is null, skip point cloud save." << std::endl;
    }
