...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mvInfoSelected(frame.mvInfoSelected),
     mvInfoCandidate(frame.mvInfoCandidate),
     mvInfoCandType(frame.mvInfoCandType),
     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    mvInfoSelected.assign(N, false);
    mvInfoCandidate.assign(N, false);
    mvInfoCandType.assign(N, 0);
...
// 
// 用法：用这个函数替换 Frame.cc 中的 ComputeStereoMatches() 函数
// 位置：约第 811-981 行
// =====================================================
...
void Frame::ComputeStereoMatches()
{
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        const vector<size_t> &vCandidates = vRowIndices[vL_int];  // ← 使用 int 索引
        ...
        if(bestDist<thOrbDist)
        {
            ...
            // ★★★ 修复 Bug 3：检查 rowRange 和 colRange 的边界 ★★★
            const cv::Mat& pyrLeft = mpORBextractorLeft->mvImagePyramid[kpL.octave];
            const cv::Mat& pyrRight = mpORBextractorRight->mvImagePyramid[kpL.octave];
            
            // 检查左图像金字塔边界
            int rowStart = static_cast<int>(scaledvL) - w;
            int rowEnd = static_cast<int>(scaledvL) + w + 1;
            int colStartL = static_cast<int>(scaleduL) - w;
            int colEndL = static_cast<int>(scaleduL) + w + 1;
            
            if(rowStart < 0 || rowEnd > pyrLeft.rows || 
               colStartL < 0 || colEndL > pyrLeft.cols)
                continue;
            
            cv::Mat IL = pyrLeft.rowRange(rowStart, rowEnd).colRange(colStartL, colEndL);
            ...
            const float iniu = scaleduR0+L-w;
            const float endu = scaleduR0+L+w+1;
            if(iniu<0 || endu >= pyrRight.cols)
                continue;

            for(int incR=-L; incR<=+L; incR++)
            {
                // ★★★ 检查右图像金字塔边界 ★★★
                int colStartR = static_cast<int>(scaleduR0) + incR - w;
                int colEndR = static_cast<int>(scaleduR0) + incR + w + 1;
                
                if(rowStart < 0 || rowEnd > pyrRight.rows ||
                   colStartR < 0 || colEndR > pyrRight.cols)
                {
                    vDists[L+incR] = FLT_MAX;
                    continue;
                }
                
                cv::Mat IR = pyrRight.rowRange(rowStart, rowEnd).colRange(colStartR, colEndR);
